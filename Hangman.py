...
def play(word):
    ...
    # Интерфейс виселицы + проверка буквы
    while guessed == False:
        print('Загаданное слово: ', *word_completion, sep='')
        print('Количество попыток:', tries)
        print('Названые буквы:', ' '.join(guessed_letters))
        print('Отгаданные слова:', ' '.join(guessed_words))
        print(display_hangman(tries))

        # Ввод буквы
        print('Введите одну букву кирилицей')
        while True:
            letter = input().upper()
            if len(letter) == 1 and letter.isalpha():
                if 1040 <= ord(letter) <= 1071 and letter not in guessed_letters:
                    guessed_letters.append(letter)
                    break
                elif 1040 <= ord(letter) <= 1071 and letter in guessed_letters:
        ...
        if word.count(letter) > 1:
            for i in range(len(word)):
                if word[i] == letter:
                    ...
                    word_completion.pop(i + 1)
                else:
                    continue
        elif word.count(letter) == 1:
            word_completion.insert(word.find(letter), letter)
            word_completion.pop(word.find(letter) + 1)
        else:
            tries -= 1
        ...
        if tries == 0:
            ...
            print('Еще разок? д/н')
            break
        elif word.upper() == ''.join(word_completion):
            print(display_hangman(tries))
            print('Вы выграли')
            print('Еще разок? д/н')
            break
